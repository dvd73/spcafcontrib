...
namespace SPCAFContrib.ReSharper.Inspection.Xml
{
    ...
    public class ConsiderOverwriteAttributeForContentType : XmlTagProblemAnalyzer
    {
        public override void Run(IXmlTag element, IHighlightingConsumer consumer)
        {
            if (element.GetProject().IsApplicableFor(this))
            {
                ...
                {
                    ConsiderOverwriteAttributeForContentTypeHighlighting errorHighlighting = new ConsiderOverwriteAttributeForContentTypeHighlighting(element);
    ...
    public class ConsiderOverwriteAttributeForContentTypeHighlighting : XmlErrorHighlighting
    {
    ...
    [QuickFix]
    public class ConsiderOverwriteAttributeForContentTypeFix : QuickFixBase
    {
        private readonly ConsiderOverwriteAttributeForContentTypeHighlighting _highlighting;
            ...
            Action<IDocument, IPsiSourceFile, IProgressIndicator> processFileAction =
                (document, psiSourceFile, indicator) =>
                {
                    if (!psiSourceFile.HasExcluded(psiSourceFile.GetSettingsStore()))
                    {
                        ...
                        {
                            new ConsiderOverwriteAttributeForContentTypeFileHandler(document, xmlFile).Run(indicator);
                        }
                    }
                };
        ...
        protected override Action<ITextControl> ExecutePsiTransaction(ISolution solution, IProgressIndicator progress)
        {
            ...
            {
                ConsiderOverwriteAttributeForContentTypeFileHandler.Fix(_highlighting.Element, solution);
            }
    ...
    public class ConsiderOverwriteAttributeForContentTypeFileHandler
    {
        private readonly IXmlFile _file;
        private readonly IDocument _document;

        public ConsiderOverwriteAttributeForContentTypeFileHandler(IDocument document, IXmlFile file)
        {
            this._file = file;
        ...
        public void Run(IProgressIndicator pi)
        {
            ...
            _file.ProcessThisAndDescendants(new RecursiveElementProcessor<IXmlTag>(
                element =>
                {
                    if (ConsiderOverwriteAttributeForContentType.IsInvalid(element)) fields.Add(element);
                }));
