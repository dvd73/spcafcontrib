...
{
    [ElementProblemAnalyzer(new[] { typeof(IObjectCreationExpression) }, HighlightingTypes = new[] { typeof(SPQueryScopeDoesNotDefinedHighlighting) })]
    [Applicability(
        IDEProjectType.SP2010FarmSolution |
        IDEProjectType.SPSandbox |
        IDEProjectType.SP2013FarmSolution |
        IDEProjectType.SPServerAPIReferenced)]
    public class SPQueryScopeDoesNotDefined : ElementProblemAnalyzer<IObjectCreationExpression>
    {
            ...
            if (sourceFile != null)
            {
                ...
                if (project != null)
                {
                    if (project.IsApplicableFor(this))
                    {
                        ...
                        {
                            consumer.AddHighlighting(new SPQueryScopeDoesNotDefinedHighlighting(element),
        ...
        public static bool IsInvalid(IObjectCreationExpression element)
        {
            ...
            if (element.IsOneOfType(new[] { ClrTypeKeys.SPQuery }))
            {
                ...
                if (element.Initializer != null)
                {
                    inInitializer = element.Initializer.InitializerElements.Any(
                        initializerElement =>
                            initializerElement is IMemberInitializer && (initializerElement as IMemberInitializer).NameIdentifier.Name == "ViewAttributes");
                }
    ...
    public class SPQueryScopeDoesNotDefinedHighlighting : IHighlighting
    {
    ...
    [QuickFix]
    public class SPQueryScopeDoesNotDefinedFix : QuickFixBase
    {
        private readonly SPQueryScopeDoesNotDefinedHighlighting _highlighting;
            ...
            Action<IDocument, IPsiSourceFile, IProgressIndicator> processFileAction =
                (document, psiSourceFile, indicator) =>
                {
                    if (!psiSourceFile.HasExcluded(psiSourceFile.GetSettingsStore()))
                    {
                        ...
                        {
                            new SPQueryScopeDoesNotDefinedHandler(document, csharpFile).Run(indicator);
                        }
                    }
                };
        ...
        protected override Action<ITextControl> ExecutePsiTransaction(ISolution solution, IProgressIndicator progress)
        {
            ...
            {
                SPQueryScopeDoesNotDefinedHandler.Fix(_highlighting.Element, solution);
            }
    ...
    public class SPQueryScopeDoesNotDefinedHandler
    {
        private readonly ICSharpFile _file;
        private readonly IDocument _document;

        public SPQueryScopeDoesNotDefinedHandler(IDocument document, ICSharpFile file)
        {
            this._file = file;
        ...
        public void Run(IProgressIndicator pi)
        {
            ...
            _file.ProcessThisAndDescendants(new RecursiveElementProcessor<IObjectCreationExpression>(
                s => { if (SPQueryScopeDoesNotDefined.IsInvalid(s)) statements.Add(s); }));
