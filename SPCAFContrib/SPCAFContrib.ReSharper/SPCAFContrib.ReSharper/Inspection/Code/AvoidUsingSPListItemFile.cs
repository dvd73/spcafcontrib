...
{
    [ElementProblemAnalyzer(new[] { typeof(IReferenceExpression) }, HighlightingTypes = new[] { typeof(AvoidUsingSPListItemFileHighlighting) })]
    [Applicability(
        IDEProjectType.SP2010FarmSolution |
        IDEProjectType.SPSandbox |
        IDEProjectType.SP2013FarmSolution |
        IDEProjectType.SPServerAPIReferenced)]
    public class AvoidUsingSPListItemFile : ElementProblemAnalyzer<IReferenceExpression>
    {
                    ...
                    if (project.IsApplicableFor(this))
                    {
                        if (IsInvalid(element))
                        {
                            consumer.AddHighlighting(
                                new AvoidUsingSPListItemFileHighlighting(element),
    ...
    public class AvoidUsingSPListItemFileHighlighting : IHighlighting
    {
        public const string CheckId = CheckIDs.Rules.Assembly.AvoidUsingSPListItemFile;
        public const string Message = "Do not use SPListItem.File";

        public IExpression Element { get; private set; }

        public AvoidUsingSPListItemFileHighlighting(IExpression element)
        {
